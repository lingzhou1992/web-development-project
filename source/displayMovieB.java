...
/* Display movies after browse by genre and/or title */
...
@WebServlet("/displayMovieB")
public class displayMovieB extends HttpServlet {
	        ...
	        try
	           {
	        	  ...
	        	  if(genre != null && letter != null && genre.compareToIgnoreCase("All") != 0 && letter.compareToIgnoreCase("All") != 0)
	        		  query = "select id from movies where title like '"+letter+"%' and id in("
	        		  		+"select movie_id from genres_in_movies where genre_id in (select id from genres where name = '"+genre+"'));";
	        	  else if(genre != null && genre.compareToIgnoreCase("All") != 0)
	        		  query = "select id from movies where id in (select movie_id from genres_in_movies where genre_id in (select id from genres where name = '"+genre+"'));";
	        	  else if(letter != null && letter.compareToIgnoreCase("All") != 0)
	        		  query = "select id from movies where title like '"+letter+"%';";
	        	  else 
	        		  query = "select id from movies;";
	        	  
	        	  ResultSet rs1 = statement.executeQuery(query);
				  while(rs1.next())
					  id_list.add(rs1.getString("id"));
				  rs1.close();
	              ...
	              if(id_list.isEmpty()){
	            	  ...
	            	  request.getRequestDispatcher("displayMovieB.jsp").forward(request, response);
	              }

	              int total_result = id_list.size();
            	  ...
            	  while(rs.next()){
		              ...
		              String title = "";
		              title += "<form method=\"get\" action= \"singleMovie\">"
