	        ...
	        try
	           {
	              ...
	              String partialtext = request.getParameter("partialtext");
	              ...
	              if(partialtext != null){
		              for(Map.Entry<String, String> kv: field.entrySet()){	            	  
		            	  if(kv.getValue().length() != 0 ){
		            		  if(kv.getKey().compareTo("year") == 0)
		            			  query = "select id from movies where "+kv.getKey()+" = '"+kv.getValue()+"';";
		            		  else if(kv.getKey().compareTo("title") == 0 || kv.getKey().compareTo("director") == 0){
		            			  String s = kv.getValue();
		            			  query = "select id from movies where "+kv.getKey()+" LIKE '%"+s+"%';";
		            		  }else{
		              ...
		              for(Map.Entry<String, String> kv: field.entrySet()){	            	  
		            	  if(kv.getValue().length() != 0 ){
		            		  if(kv.getKey().compareTo("title") == 0 ||kv.getKey().compareTo("year") == 0 || kv.getKey().compareTo("director") == 0){
		            			  query = "select id from movies where "+kv.getKey()+" = '"+kv.getValue()+"';";
		            		  }else{
		            			  query = "select id from movies where id in (select movie_id from stars_in_movies where star_id in (select id from stars where "+kv.getKey()+" = '"+kv.getValue()+"'));";
		            		  }
	            			  ResultSet rs = statement.executeQuery(query);
	            			  while(rs.next())
	            				  id_list.add(rs.getString("id"));
	              ...
	              for(String s: id_list){
	            	  if(id_count.containsKey(s))
	            		  id_count.put(s,id_count.get(s)+1);
					  else
						  id_count.put(s,1);
	              }
	              ...
	              request.setAttribute("partialtext", partialtext);
